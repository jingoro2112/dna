...
// buggy inconsistent mess, banged on to work -Curt
...
{
	return get_string_property_utf8(device, CFSTR(kIOHIDSerialNumberKey), (char *)buf, len);
}

static int get_manufacturer_string(IOHIDDeviceRef device, wchar_t *buf, size_t len)
{
	return get_string_property_utf8(device, CFSTR(kIOHIDManufacturerKey), (char *)buf, len);
}

static int get_product_string(IOHIDDeviceRef device, wchar_t *buf, size_t len)
{
	return get_string_property_utf8(device, CFSTR(kIOHIDProductKey), (char *)buf, len);
}
...
hid_device * HID_API_EXPORT hid_open(unsigned short vendor_id, unsigned short product_id, wchar_t *serial_number)
{
	...
	devs = hid_enumerate(0,0);//vendor_id, product_id);
	cur_dev = devs;
	while (cur_dev) {
...
int HID_API_EXPORT_CALL hid_get_manufacturer_string( hid_device *dev, char *string, size_t maxlen)
{
	CFStringRef str = IOHIDDeviceGetProperty( dev->device_handle, CFSTR(kIOHIDManufacturerKey) );
	if ( str )
	{
